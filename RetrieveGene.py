...
#-------------------------------------------------------------------------------
# INPUT file names:
structIDSource = 'structIDs_all.csv' # 'layerIDs.csv' # 'structureIDs.csv'
...
def download_mouse_unionizes(geneEntrezID,structureIDs,doReduced='medium',writeOut=False):
    ...
    # Set defaults:
    if doReduced == 'struc_gene_density':
        print("Only retrieving a minimal subset of results from the queries")
        optionsString = "[only$eq'structures.id,data_sets.idgenes.entrez_id,structure_unionizes.expression_density']"
    elif doReduced=='medium':
...
#---------------------------------------------------------------------------
structureIDs = np.genfromtxt(structIDSource,delimiter=',', dtype=np.int)
...
# parallel downloading through joblib.Parallel, alter num_jobs for more simultaneous downloads
num_jobs = 6
num_structures = len(structureIDs)

def structure_details_wrapper(struc_ids):
    return GetStructureDetails(struc_ids, filterFields, 1)
struct_list = Parallel(n_jobs=num_jobs)(delayed(structure_details_wrapper)(sub_list)
        for sub_list
        in np.split(structureIDs, list(range(0,num_structures,num_structures//num_jobs))[1:-1]))

structs = sum(struct_list, [])
...
if not os.file.exists(json_file_name):
    # Parallelize download using num_jobs and num_structures from the previous parallelization
    def download_mouse_unionizes_wrapper(struc_ids):
        return download_mouse_unionizes([],struc_ids,doReduced='medium',writeOut=False)
    unionizes = Parallel(n_jobs=num_jobs)(delayed(download_mouse_unionizes_wrapper)(sub_list)
            for sub_list
            in np.split(structureIDs, list(range(0,num_structures,num_structures//num_jobs))[1:-1]))
    unionizes = sum(unionizes, [])

    # unionizes = download_mouse_unionizes([],structureIDs,doReduced='medium',writeOut=False)
